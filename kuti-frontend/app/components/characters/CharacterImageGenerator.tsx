...
export function CharacterImageGenerator({
  ...
  const previewPrompt = useMemo(() => {
    ...
    if (character.physicalDescription)
      parts.push(
        `${t("generation.promptLabels.physical")}: ${character.physicalDescription}`,
      );
    if (character.costumeElementsJson?.length) {
      parts.push(
        `${t("generation.promptLabels.wearing")}: ${character.costumeElementsJson
          .slice(0, 5)
          .join(", ")}`,
      );
    }
    if (character.colorPaletteJson?.length) {
      parts.push(
        `${t("generation.promptLabels.colors")}: ${character.colorPaletteJson
          .slice(0, 3)
          .join(", ")}`,
      );
    }
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
          ...
          <div className="flex flex-col gap-4">
            ...
            {generateMutation.error && (
              <ErrorState
                message={apiErrorMessage(generateMutation.error as unknown)}
              />
            )}
            ...
            <div>
              ...
              <ToggleGroup
                ...
                value={strategy}
                onValueChange={(value) =>
                  value && setStrategy(value as Strategy)
                }
                className="grid w-full grid-cols-3"
              >
                {STRATEGIES.map((s) => (
                  <ToggleGroupItem key={s} value={s} className="w-full">
