...
function BreadcrumbDropdown({
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={onOpenChange}>
      ...
      <span className="max-w-[260px] truncate text-sm font-medium text-foreground lg:hidden">
        {label}
      </span>
...
export function StoryBreadcrumb({
  ...
  const chapterItems: DropdownItem[] = tomeChapters.map((chapter, index) => ({
    ...
    onClick: () =>
      navigate(
        `/projects/${projectId}/story/${currentTomeId}/chapters/${chapter.id}`,
      ),
  }));
  ...
  const sceneItems: DropdownItem[] = chapterScenes.map((scene, index) => ({
    ...
    onClick: () =>
      navigate(
        `/projects/${projectId}/story/${currentTomeId}/scenes/${scene.id}`,
      ),
  }));
