...
export default function CharactersRoute() {
  ...
  const create = useMutation({
    mutationFn: (body: { name: string }) => api.createCharacter(projectId, body),
    onSuccess: (character) => { setSelectedId(character.id); invalidateWorkspace(projectId); },
  });
  const update = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Partial<Character> }) => api.updateCharacter(projectId, id, body),
  ...
  return (
    <AppShell>
      <div className="page-header"><div><h1>Characters</h1><p>Profiles, relations and voice notes are persisted directly through the backend.</p></div></div>
        ...
        <Panel>
          ...
          <div className="list">
            {(characters.data?.items || []).map((character) => (
              <button key={character.id} className={`list-item ${selected?.id === character.id ? "selected" : ""}`} onClick={() => setSelectedId(character.id)}>
                <strong>{character.name}</strong>
                <small>{character.narrative_role || character.slug}</small>
        ...
        <Panel>
          ...
          {detail.data ? (
            <div className="grid">
              ...
              <div className="list">{detail.data.relations.map((item) => <div className="list-item" key={item.id}><strong>{item.relation_type} · {item.strength}</strong><small>{item.source_character_id} to {item.target_character_id}</small></div>)}</div>
              <SectionTitle title="Voice samples" meta={`${detail.data.voice_samples.length}`} />
              <div className="list">{detail.data.voice_samples.map((item) => <div className="list-item" key={item.id}><strong>{item.label}</strong><small>{item.voice_notes || item.asset_path}</small></div>)}</div>
            </div>
          ) : null}
        </Panel>
      </div>
...
function CharacterForm({ character, saving, onSave }: { character: Character; saving: boolean; onSave: (body: Partial<Character>) => void }) {
  ...
  return (
    <form className="form" onSubmit={(event) => { event.preventDefault(); onSave({ name, alias, narrative_role: role, description, physical_description: physical, key_traits_json: csv(traits), color_palette_json: csv(palette), costume_elements_json: csv(costume), personality, narrative_arc: arc, tags_json: csv(tags) }); }}>
      <div className="grid two"><Field label="Name"><input value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Alias"><input value={alias} onChange={(event) => setAlias(event.target.value)} /></Field></div>
      ...
      <div className="grid two"><Field label="Traits"><input value={traits} onChange={(event) => setTraits(event.target.value)} /></Field><Field label="Palette"><input value={palette} onChange={(event) => setPalette(event.target.value)} /></Field></div>
...
function RelationForm({ characters, selectedId, onSubmit }: { characters: Character[]; selectedId: string; onSubmit: (body: { target_character_id: string; relation_type: string; strength: number }) => void }) {
  ...
  return <form className="form" onSubmit={(event) => { event.preventDefault(); if (target) onSubmit({ target_character_id: target, relation_type: type, strength }); }}>
