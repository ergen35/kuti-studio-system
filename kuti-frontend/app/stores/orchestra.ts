...
export const useOrchestraStore = create<OrchestraState>()(
  persist(
    (set, get) => ({
      ...
      selectChapter: (id: string | null) =>
        set({
          selectedSceneId: null,
          selectedChapterId: id,
          selectedTomeId: null,
        }),
      selectTome: (id: string | null) =>
        set({
          selectedSceneId: null,
          selectedChapterId: null,
          selectedTomeId: id,
      ...
      zoomViewport: (factor, centerX, centerY) =>
        set((state) => {
          const newZoom = Math.max(
            0.2,
            Math.min(3, state.viewport.zoom * factor),
          );
      ...
      fitToBounds: (
        ...
        const targetX =
          canvasWidth / 2 - (bounds.x + bounds.width / 2) * targetZoom;
        const targetY =
          canvasHeight / 2 - (bounds.y + bounds.height / 2) * targetZoom;
